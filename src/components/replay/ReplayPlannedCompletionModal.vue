        ...
        <template v-if="datePoints.length">
          <section class="replay-completion-timeline-section" aria-label="计划验证日期时间轴">
            <div class="replay-completion-range-fields">
              ...
              <small>默认显示最新 3 个计划日期</small>
            </div>

            <div ref="timelineScrollRef" class="replay-completion-timeline-scroll">
              <div class="replay-completion-timeline" :style="timelineWidthStyle">
                <div class="replay-completion-columns">
                  <div
                    ...
                    :class="index >= startIndex && index <= endIndex ? 'is-selected' : 'is-outside'"
                  >
                ...
                <div class="replay-completion-slider-shell" :style="sliderInsetStyle">
                  <div class="replay-completion-slider-track"></div>
                  <div class="replay-completion-slider-selection" :style="selectionStyle"></div>
                  <input
          ...
          <section v-if="dashboard" class="replay-completion-results" aria-label="计划完成统计结果">
            <div class="replay-completion-overview">
...
function barHeight(value) {
  return `${Math.max(18, Math.round((Number(value) || 0) / maxPlannedCount.value * 112))}px`
}
...
async function captureSnapshot() {
  ...
  try {
    ...
    else snapshotMessage.value = '快照已保存，但图片复制失败'
  } catch {
    await effectFinished
...
function indexOfDate(date, fallback) {
  const found = datePoints.value.findIndex(point => point.date === date)
...
function synchronizeRange(startDate, endDate) {
  ...
  startIndex.value = indexOfDate(startDate, startIndex.value)
  endIndex.value = indexOfDate(endDate, endIndex.value)
}
...
async function loadDashboard(startDate, endDate, synchronizeEffective = false) {
  ...
  error.value = ''
  try {
    const result = await getReplayCompletionDashboard({ startDate, endDate })
...
async function initialize() {
  ...
  try {
    ...
    if (!datePoints.value.length) return
...
async function applyInputRange() {
  const selectedStartIndex = datePoints.value.findIndex(point => point.date === startDateInput.value)
  const selectedEndIndex = datePoints.value.findIndex(point => point.date === endDateInput.value)
...
watch(() => props.open, (open) => {
  if (open) initialize()
  else resetSnapshotFeedback()
}, { immediate: true })

onBeforeUnmount(resetSnapshotFeedback)
</script>

<style scoped>
.replay-completion-mask{position:fixed;inset:0;z-index:1300;display:flex;align-items:center;justify-content:center;padding:24px;background:rgba(15,23,42,.48);backdrop-filter:blur(2px)}
.replay-completion-modal{position:relative;display:flex;flex-direction:column;width:min(1500px,calc(100vw - 48px));height:min(900px,calc(100vh - 48px));overflow:hidden;border:1px solid #dbe3ef;border-radius:14px;background:var(--bg-card,#fff);color:var(--text-primary,#172033);box-shadow:0 24px 80px rgba(15,23,42,.28)}
.replay-completion-header{display:flex;align-items:center;justify-content:space-between;padding:18px 22px;border-bottom:1px solid #e5eaf1;background:linear-gradient(180deg,#fff,#fbfcfe)}
...
.replay-completion-body{flex:1;min-height:0;overflow:auto;padding:18px 22px 24px}.replay-completion-error{padding:10px 12px;border-radius:8px;background:#fff1f0;color:#b42318;font-size:13px}.replay-completion-state{padding:36px;text-align:center;color:#718096}
.replay-completion-timeline-section{padding:14px 0 18px;border-bottom:1px solid #e8edf4}.replay-completion-range-fields{display:flex;align-items:end;gap:10px;margin-bottom:16px}.replay-completion-range-fields label{display:grid;gap:5px;color:#667085;font-size:12px}.replay-completion-range-fields select{width:150px;height:34px;padding:0 30px 0 10px;border:1px solid #cfd8e6;border-radius:6px;background:var(--bg-card,#fff);color:inherit;cursor:pointer}.replay-completion-range-fields button,.replay-completion-drawer footer button{height:34px;padding:0 14px;border:1px solid #176fd1;border-radius:6px;background:#176fd1;color:#fff;cursor:pointer}.replay-completion-range-fields small{margin-left:auto;color:#8491a5;font-size:12px}
.replay-completion-timeline-scroll{overflow-x:auto;overflow-y:hidden;padding:3px 0 12px}.replay-completion-timeline{position:relative;width:100%;padding-bottom:38px}.replay-completion-columns{display:grid;grid-template-columns:repeat(var(--timeline-count),minmax(96px,1fr));height:184px;border-bottom:1px solid #d7dfeb;background:linear-gradient(180deg,rgba(239,245,252,.55),rgba(255,255,255,0))}
.replay-completion-column{display:grid;min-width:0;grid-template-rows:148px 12px 24px;justify-items:center;align-items:end;opacity:.28;transition:opacity .16s}.replay-completion-column.is-selected{opacity:1}.replay-completion-bar-space{display:flex;align-items:end;justify-content:center;width:100%;height:148px}.replay-completion-bar-stack{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;height:100%}.replay-completion-count{margin-bottom:5px;color:#29476f;font-size:12px;font-weight:700;line-height:18px}.replay-completion-bar{display:block;width:34px;min-height:18px;border-radius:5px 5px 1px 1px;background:#2f7edb;box-shadow:inset 0 1px rgba(255,255,255,.28)}.replay-completion-point{align-self:center;width:7px;height:7px;border:2px solid #fff;border-radius:50%;background:#2f7edb;box-shadow:0 0 0 1px #2f7edb}.replay-completion-column time{align-self:start;color:#667085;font-size:11px;white-space:nowrap}
.replay-completion-slider-shell{position:absolute;bottom:11px;height:18px}.replay-completion-slider-track,.replay-completion-slider-selection{position:absolute;top:8px;height:3px;border-radius:2px}.replay-completion-slider-track{left:0;right:0;background:#d9e2ef}.replay-completion-slider-selection{background:#2f7edb}.replay-completion-range{position:absolute;inset:0;width:100%;height:18px;margin:0;pointer-events:none;appearance:none;background:transparent}.replay-completion-range::-webkit-slider-thumb{width:15px;height:15px;border:3px solid #fff;border-radius:50%;background:#2f7edb;box-shadow:0 0 0 1px #2f7edb,0 2px 5px rgba(23,111,209,.25);pointer-events:auto;appearance:none;cursor:grab}.replay-completion-range::-moz-range-thumb{width:10px;height:10px;border:3px solid #fff;border-radius:50%;background:#2f7edb;box-shadow:0 0 0 1px #2f7edb;pointer-events:auto;cursor:grab}.replay-completion-range-start{z-index:2}.replay-completion-range-end{z-index:3}
.replay-completion-results{padding-top:18px}.replay-completion-overview{display:grid;grid-template-columns:repeat(6,minmax(118px,1fr));gap:10px;margin-bottom:16px}.replay-completion-overview>div{display:grid;gap:5px;padding:12px 14px;border:1px solid #e2e8f1;border-radius:8px;background:#f8fafc}.replay-completion-overview span{color:#718096;font-size:12px}.replay-completion-overview strong{font-size:20px;color:#1e3555}
.replay-completion-table-stage{position:relative;border-radius:9px}.replay-completion-table-wrap{max-height:min(420px,45vh);overflow:auto;border:1px solid #dfe6ef;border-radius:9px}.replay-completion-table{width:100%;min-width:850px;border-collapse:collapse}.replay-completion-table th,.replay-completion-table td{height:42px;padding:0 13px;border-bottom:1px solid #e8edf3;text-align:right;font-size:13px}.replay-completion-table thead th{position:sticky;top:0;z-index:1;background:#f2f6fb;color:#536176;font-weight:600}.replay-completion-table th:first-child{text-align:left}.replay-completion-table tbody:last-child tr:last-child>*{border-bottom:0}.replay-completion-group-row{background:#f9fbfd}.replay-completion-group-row th{font-weight:700;color:#1f3656}.replay-completion-developer-row th span{display:inline-block;padding-left:22px;color:#5f6e82}.replay-completion-table td button{min-width:36px;padding:4px 9px;border:0;border-radius:5px;background:#edf5ff;color:#176fd1;font-weight:700;cursor:pointer}.replay-completion-table td button:hover{background:#dcecff}
...
@media(max-width:900px){.replay-completion-mask{padding:10px}.replay-completion-modal{width:calc(100vw - 20px);height:calc(100vh - 20px)}.replay-completion-overview{grid-template-columns:repeat(3,1fr)}.replay-completion-drawer{width:min(560px,88vw)}.replay-completion-range-fields{flex-wrap:wrap}.replay-completion-range-fields small{width:100%;margin-left:0}.replay-completion-group-toolbar{align-items:flex-start;flex-wrap:wrap}.replay-completion-group-tabs{width:100%}.replay-completion-snapshot-actions{width:100%}.replay-completion-snapshot-message{margin-right:auto;white-space:normal}}
